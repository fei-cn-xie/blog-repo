...
def seed_digest(
    *, device_id: str, custom_factor: str, ip: str | None, conv_round: int | None,
    ...
    """seed 的 32 bit 摘要。运行期按 flight 校验两侧动态因子是否一致（不泄露 seed）。"""
...
def scramble_tensor(t: Tensor, *, seed: int) -> Tensor:
    """就地加扰**一个**张量（一个向量矩阵），返回入参本身。

    dtype 不是 16 bit 浮点 / 不能按 16 bit 重解释 → ValueError。
    """

def descramble_tensor(t: Tensor, *, seed: int) -> Tensor:
    """scramble_tensor 的严格逆。seed 必须与对应的调用完全一致。"""

def scramble_payload(d: Mapping[str, Tensor], *, seed: int) -> list[str]:
    """对字典中所有 16 bit 浮点张量就地加扰；返回**实际处理**的键列表。

    非 16 bit 浮点的键按 dtype 规则跳过（不报错）——跳过的键 =
    set(d) - set(返回值)，调用方可据此断言。
    """

def descramble_payload(d: Mapping[str, Tensor], *, seed: int) -> list[str]:
    """同上，逆操作。d 的键集合必须与发送侧一致（由 wire meta 保证）。"""
```
...
**为什么"跳过"不设参数**：跳过的集合**完全由 dtype 决定**，不需要调用方声明。早期版本有一个 `skip={"mrope_positions"}` 参数，它的问题和 `op` 一样——**是一个没有任何校验覆盖的自由度**：若两侧的 `skip` 不一致，被跳过的键是 16 bit 浮点时会出现"一侧加扰、另一侧不加扰"的**静默损坏**（`seed_digest` 不含 `skip`）。而 v1 唯一需要跳过的 `mrope_positions` 是 int64，dtype 规则天然推出同样结果。

**调用方如何确认"没有漏处理"**：`返回值` 是实际处理的键，跳过的键 = `set(d) - set(返回值)`。要强断言就写：

```python
processed = ps.scramble_payload(d, seed=seed)
...
```python
def scramble_payload(d,          # 位置参数：张量字典（单张量版则是张量本身）
...
| 符号 | 是什么 | 为什么这么设计 |
|---|---|---|
| `*` | Python 的**关键字-only 分隔符**，不是参数。它右边的参数不能用位置传（`scramble_payload(d, 12345)` → `TypeError`） | ① 防顺序写错——位置传参时把别的参数当成 seed，就是"用错的 seed 解扰"，属于本设计最怕的静默错误；② 调用点自解释（`seed=seed` 比裸整数清楚）；③ **`derive_seed(**factors)` 能工作的前提**——字典展开只产生关键字参数，所以五个因子必须关键字-only |
| `seed` | 必填、只能按关键字传 | 算子只做"向量 + seed → 向量"这一步；"因子 → seed"由 `derive_seed` 单独完成（见下） |
...
| 曾有的参数 | 为什么删 |
|---|---|
| `factors={...}` | ① 两件事本该分开：`derive_seed`（因子 → seed）与 `scramble_tensor`（向量 + seed → 向量）是独立两步，算子收 `factors` 等于把派生藏进内部，调用点上看不出"这批用的是哪个 seed"；② **生产路径零调用者**——通信层只从 `CommRequest.scramble_seed` 拿到一个 int，既没有 `SchedulerOutput` 也没有因子；③ 连带少两条校验规则（`factors`/`seed` 二选一、键集合校验） |
| `op="iadd"｜"xor"` | `op` 不参与 `derive_seed`，所以 `seed_digest` / `conformance_digest` **都覆盖不到它**：一侧用加法加扰、另一侧用异或解扰 → 数据损坏，而两道校验全部通过。v1 固定加法（§9.4） |
| `skip={"mrope_positions"}` | 跳过的集合**完全由 dtype 决定**，不需要调用方声明。若两侧 `skip` 不一致，且被跳过的键恰好是 16 bit 浮点，就会出现"一侧加扰、另一侧不加扰"的**静默损坏**（`seed_digest` 不含 `skip`）。改为按 dtype 规则跳过（见上表） |

派生因此显式写在调用点上，而不是藏进算子：

```python
seed = ps.derive_seed(device_id=DEV_ID, custom_factor=CUSTOM,
                      ip=client_ip, conv_round=turn, session_id=sid)
ps.scramble_payload(send_dict, seed=seed)      # 就地
```

这一行换来的是**数据流在调用点上可见**——本设计反复强调"seed 必须两侧一致、每 flight 一个"，把派生写出来比藏进算子更贴合这一点。派生只做一次：worker 的 helper 算好后放进 `CommRequest`，所有收发点复用同一个 int（§8.2 ④）。

**输入校验（fail fast）**

| 校验 | 行为 |
|---|---|
| `seed` 必须是 `int` | `None` / 其他类型 → `ValueError`（否则会在 `seed & 0xFFFF` 处抛出不直观的 `TypeError`） |
...
把 16 bit 浮点按位解释为整数，再加上 seed 的低 16 位（模 `2¹⁶`）：

```
加扰：Y = (u16(X) + k) mod 2¹⁶        k = seed & 0xFFFF
解扰：u16(X) = (Y − k) mod 2¹⁶
```

同语义的可选实现还有"异或"：`u16(x) ^ (seed & 0xFFFF)`，同样严格无损、同样零额外内存与单核。**v1 固定用加法**，不开放选择——理由见 §2（`op` 不一致没有任何校验能发现），异或留作将来可选。
...
# 先派生（每 flight 一次；本项目里这一步在 worker 的 helper 中）
seed = ps.derive_seed(**factors)

# 发送侧
ps.scramble_payload(send_dict, seed=seed)                # 就地

# 接收侧（另一进程 / 另一节点，用同一组因子独立派生出同一个 seed）
ps.descramble_payload(recv_dict, seed=ps.derive_seed(**factors))   # 就地
...
5. 穷举 65536 个 bit 模式往返（**平台回绕守卫**）
6. bf16 / fp16，含 NaN / ±Inf / 次正规数；非连续张量应抛错并提示 `.contiguous()`
7. 3D 张量（`hc_mult > 1`）与 2D 张量同样正确
8. `scramble_payload` 的返回值与"按 dtype 跳过"行为；跳过的键 = `set(d) - set(返回值)`
...
11. **等价性**：`scramble_payload(d, seed=s)` 的结果与对 `d` 中每个 16 bit 浮点张量逐一调 `scramble_tensor` 逐 bit 相同（证明"薄封装"契约），返回的键列表 == 被处理的键集
12. 非 16 bit dtype / 非连续张量 → `ValueError`
...
| ③ | 同函数 **非 merge 分支** | `descramble_payload(dict, seed=…)` | `tensor_dict` 中逐键的 irecv 缓冲 + `request.scramble_seed` | 同样早于 TP 广播（该分支下 `broadcast_postprocess` 会原地广播这些缓冲） |
...
| 挂载点 | `EdgeCloudConfig.__init__` 里 `self.payload_scramble = PayloadScrambleConfig(user_config.get("payload_scramble"))` |
| 访问方式 | `get_ascend_config().edge_cloud_config.payload_scramble`——配置是**进程级单例**（`ascend_config.py:1094`），`model_runner_v1.py:945-950` 已是这个取法 |
| 配置方 | **边云两侧都要配**；但"必须一致的字段"只占一部分（见 §9.3 分组） |
...
class PayloadScrambleConfig:
    ...
    **构造即表示"本部署启用了加扰能力"**：只有配置里出现 ``payload_scramble`` 键时
    ...
    def local_factors(self) -> dict[str, str]:
        """本进程侧的**静态**因子，可直接展开给模块的两个摘要函数。

        >>> ps.conformance_digest(**cfg.local_factors())
        """
    ...
    def factors(self, *, ip: str | None, conv_round: int | None,
                session_id: str) -> dict:
        """拼装全部 5 个因子：静态来自配置，动态由调用方提供。

        >>> seed = ps.derive_seed(**cfg.factors(ip=ip, conv_round=n, session_id=sid))

        存在的意义是**把因子名单集中在一处**——调用点不会因为写错键名而拿到
        不直观的 TypeError（这正是早期版本用 factors 键集合校验去防的那类错误，
        现在从设计上不会发生）。
        """
        return {**self.local_factors(), "ip": ip,
                "conv_round": conv_round, "session_id": session_id}
...
| ② | `enabled=True` 且启用了 MTP / Eagle3 | **WARN** | v1 不覆盖 draft wire（§9.12），得让人知道 draft 载荷还是明文 |
| ③ | `role="cloud"` 且显式配了 `enabled` | **忽略 + WARN**（不报错） | 开关权威只在边侧（§9.10）。**不报错是刻意的**：若要求两侧都配且一致，就把"两侧配置必须对齐"这个一致性问题又引回来了——而那正是本功能要消除的东西 |

**`resolve_custom_factor()` 的调用时机**：worker 启动时显式调一次（`NPUWorker.init_device()`，与 `init_edge_cloud_tensor_meta()` 同一处），把"文件不可读"变成**启动期 fail fast**；之后每 flight 直接读缓存。
...
- `seed_digest` / `conformance_digest` 都是对 **seed** 取摘要，而 `op` 不参与 `derive_seed`
- 因此一侧用加法加扰、另一侧用异或解扰时，数据损坏而**两道校验都会通过**

**已决定：v1 固定用加法，配置层与 API 层都不开放这个选择**（§2 已同步去掉 `op` 参数）。异或留作将来的可选实现——要重新开放它，必须**连同启动握手的载荷一起加回**（交换内容变成 `(enabled, 算法标识, conformance_digest)`）。
...
| # | 规则 |
|---|---|
| 1 | A 组字段必须存在且 `strip()` 后非空 |
| 2 | `custom_factor` 与 `custom_factor_file` **恰好给一个**（都给或都不给 → 报错） |
| 3 | `custom_factor_file` 指定的文件必须可读，且 `strip()` 后非空 |
| 4 | `conv_round_semantics` 必须落在允许集合内——口径歧义会让边侧取到不同轮次 |
| 5 | `ip_header_priority` 为空且 `trust_socket_peer=False` → 报错（`ip` 将恒为降级常量，动态因子退化成静态） |
...
云侧把 `"role": "edge"` 改成 `"cloud"`，**A 组字段逐字段相同**；B 组字段云侧可以留空或省略。
